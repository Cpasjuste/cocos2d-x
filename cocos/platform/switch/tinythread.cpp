...
#if defined(_TTHREAD_POSIX_) || defined(_TTHREAD_SWITCH_)
  #include <unistd.h>
  #include <map>
...
#if defined(_TTHREAD_SWITCH_)
static thread::id _thread_t_to_ID(const thrd_t &aHandle)
{
  static mutex idMapLock;
  static std::map<thrd_t, unsigned long int> idMap;
  static unsigned long int idCount(1);
...
#elif defined(_TTHREAD_POSIX_) || defined(_TTHREAD_SWITCH_)
...
#elif defined(_TTHREAD_SWITCH_)
  if(thrd_create(&mHandle, (thrd_start_t)wrapper_function, (void *) ti) != thrd_success)
    mHandle = 0;
...
#elif defined(_TTHREAD_SWITCH_)
  return _thread_t_to_ID(mHandle);
#elif defined(_TTHREAD_POSIX_)
...
#elif defined(_TTHREAD_SWITCH_)
  return _thread_t_to_ID(thrd_current());
#elif defined(_TTHREAD_POSIX_)
